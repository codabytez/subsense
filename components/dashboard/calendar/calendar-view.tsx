  ...
  return (
    <FadeIn className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        ...
        {/* View toggle */}
        <div className="flex items-center bg-surface border border-border rounded-xl p-1 shrink-0 self-start">
          {VIEWS.map((v) => (
            <button
              key={v}
              onClick={() => setView(v)}
              className={cn(
                "relative px-3 sm:px-5 py-2 text-xs font-bold tracking-widest uppercase rounded-lg transition-colors duration-200",
